...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        #add mutation differ
        self.word_embeddings2 = nn.Embedding(config.vocab_size, config.hidden_size, padding_idx=0)
        self.position_embeddings = nn.Embedding(config.max_position_embeddings, config.hidden_size)
        self.token_type_embeddings = nn.Embedding(config.type_vocab_size, config.hidden_size)
        self.ref_to_alt_embeddings = nn.Embedding(config.vocab_size2, config.hidden_size)
        # self.LayerNorm is not snake-cased to stick with TensorFlow model variable name and be able to load
        # any TensorFlow checkpoint file
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)

    def forward(self, input_ids=None, input_ids2=None,token_type_ids=None, position_ids=None, inputs_embeds=None,ref_to_alt_ids=None,):
        ...
        if inputs_embeds is None:
            ...
            inputs_embeds2 = self.word_embeddings2(input_ids2)
            ref_to_alt_embeddings = self.ref_to_alt_embeddings(ref_to_alt_ids)
            ref_to_alt_embeddings=ref_to_alt_embeddings.view(ref_to_alt_embeddings.size(0), -1)
        position_embeddings = self.position_embeddings(position_ids)
        token_type_embeddings = self.token_type_embeddings(token_type_ids)

        embeddings = inputs_embeds + position_embeddings + token_type_embeddings + (inputs_embeds-inputs_embeds2)
        embeddings = self.LayerNorm(embeddings)
        embeddings = self.dropout(embeddings)
        return embeddings,ref_to_alt_embeddings
...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if att_head_mask is not None:
            attention_probs = attention_probs * att_head_mask
        ...
        #with torch.no_grad():
        #   context_layer = torch.matmul(attention_probs, value_layer)
        #   context_layer.requires_grad = True
...
class BertAttention(nn.Module):
    ...
    def forward(
        ...
        self_outputs = self.self(
            hidden_states, attention_mask, head_mask, encoder_hidden_states, encoder_attention_mask,tmp_score,att_head_mask,
        )
...
class BertLayer(nn.Module):
    ...
    def forward(
        ...
        self_attention_outputs = self.attention(hidden_states, attention_mask, head_mask,tmp_score=tmp_score,att_head_mask=att_head_mask)
...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        all_hidden_states = ()
        all_attentions = ()
        kmer_avg_all=torch.zeros([hidden_states.shape[0],1,hidden_states.shape[-1]])
        if tmp_score is None:
            kmer_avg_all=torch.zeros([hidden_states.shape[0],1,hidden_states.shape[-1]])
        else:
            kmer_avg_all=torch.zeros([tmp_score.shape[0],1,hidden_states.shape[-1]])
            hidden_states= hidden_states.expand(tmp_score.shape[0],hidden_states.shape[1],hidden_states.shape[-1])        
        for i, layer_module in enumerate(self.layer):
            if self.output_hidden_states:
                all_hidden_states = all_hidden_states + (hidden_states,)
            if att_head_mask is None:
                if i==tar_layer:
                    layer_outputs = layer_module(hidden_states, attention_mask, head_mask[i], encoder_hidden_states, encoder_attention_mask,tmp_score)
                else:
            ...
            # )            
                ...
                att_head_mask_one_layer = att_head_mask[i]
                layer_outputs = layer_module(hidden_states, attention_mask, head_mask[i], encoder_hidden_states, encoder_attention_mask,att_head_mask=att_head_mask_one_layer)
                hidden_states = layer_outputs[0]
                if self.output_attentions:
                    all_attentions = all_attentions + (layer_outputs[1],)
            kmer_avg=(torch.sum(hidden_states[:,49:52],dim=1)/3).unsqueeze(1)
            kmer_avg_all =torch.cat([kmer_avg_all,kmer_avg],dim=1)
        kmer_avg_all=kmer_avg_all[:,1:,:]
        kmer_avg_all=kmer_avg_all.view(kmer_avg_all.shape[0],kmer_avg_all.shape[1],12,64)
        sub_mo1=self.res(kmer_avg_all)
        ...
        return outputs,sub_mo1  # last-layer hidden state, (all hidden states), (all attentions)
...
class BertGlobal(nn.Module):
    def __init__(self, config):
        super().__init__()
        # self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        # self.dense_atten = nn.Linear(config.hidden_size, config.num_attention_heads)
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
    ...
    def forward(self, hidden_states):
        ...
        # # to the first token.
        # # hidden_states=torch.sum(hidden_states[:,:],dim=1)/98
        # # hidden_states=hidden_states[:,0]
        # atted_output = self.dense(hidden_states)
        # atted_output = self.activation(atted_output)
        # atted_output = self.dense_atten(atted_output)
        # atted_output = atted_output.permute(1, 0, 2)
        # atted_output = nn.Softmax(dim=0)(atted_output)
        # atted_output = atted_output.permute(1, 0, 2)        
        # # pooled_output = self.activation(pooled_output)
        # atted_output=torch.sum(torch.bmm(atted_output.permute(0, 2, 1), hidden_states), dim=1)
        # atted_output=self.LayerNorm(atted_output)
        # atted_output = torch.sum(hidden_states,dim=1)
        # atted_output=torch.sum(hidden_states[:,:],dim=1)/98
        atted_output = hidden_states[:,0]
        atted_output = self.LayerNorm(atted_output)
        
        return atted_output


class BertRef2Alt(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        # self.activation = nn.Tanh()  
        # self.LayerNorm = BertLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        # self.activation2=torch.nn.functional.sigmoid
        self.LayerNorm2 = BertLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
    def forward(self, hidden_states, hidden_states2):
        alted_output = self.dense(hidden_states)
        #first_token_tensor = hidden_states[:, 0]
        #pooled_output = self.dense(first_token_tensor)
        # alted_output = self.activation(alted_output)
        # alted_output = alted_output*hidden_states
        alted_output=self.LayerNorm2(alted_output)
        # alted_p = self.activation2(hidden_states2+alted_output)
        # line2_raw=torch.sum(line2_raw[:,:],dim=1)/100
        # alted_output=alted_p*hidden_states+(1-alted_p)*line2_raw
        alted_output=alted_output+hidden_states2
        return alted_output

class BertCatPooler(nn.Module):
    def __init__(self, config):
        super().__init__()
        # self.dense = nn.Linear(config.hidden_size, config.hidden_size)
    ...
    def forward(self, hidden_states,hidden_states2):
        # catpooler = self.dense(hidden_states)
        # #first_token_tensor = hidden_states[:, 0]
        # #pooled_output = self.dense(first_token_tensor)
        # catpooler = self.activation(catpooler)
        # catpooler = self.dense2(catpooler)
        # catpooler = self.activation(catpooler)
        # catpooler = catpooler.view(catpooler.size(0), -1)      
        # catpooler=torch.cat([hidden_states2,hidden_states],dim=1) 
        # F = torch.sigmoid(self.dense(hidden_states)+self.dense2(hidden_states2))
        # catpooler = F*hidden_states+(1-F)*hidden_states2
        # catpooler = hidden_states
        catpooler=torch.cat([hidden_states,hidden_states2],dim=1)
        # catpooler=hidden_states+hidden_states2
        return catpooler
...
class BertModel(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.global1 = BertGlobal(config)
        self.ref2alter=BertRef2Alt(config)
        self.catpooler=BertCatPooler(config)
...
#            encoder_extended_attention_mask = (1.0 - encoder_extended_attention_mask) * -10000.0
        ...
        head_mask = self.get_head_mask(head_mask, self.config.num_hidden_layers) #[None, None, None, None, None, None, None, None, None, None, None, None]
...
#            head_mask = [None] * self.config.num_hidden_layers
        ...
        embedding_output,ref_to_alt_embeddings = self.embeddings(
            input_ids=input_ids, input_ids2=input_ids2,position_ids=position_ids, token_type_ids=token_type_ids, inputs_embeds=inputs_embeds, ref_to_alt_ids=ref_to_alt_ids,
        )

        encoder_outputs,res_blok = self.encoder(
            embedding_output,
            ...
            tar_layer=tar_layer,
            tmp_score=tmp_score,
            att_head_mask=att_head_mask,
        )
        sequence_output = encoder_outputs[0]
        
        pooled_output = self.global1(sequence_output)

        alted_output = self.ref2alter(ref_to_alt_embeddings,pooled_output)
        # alted_output = alted_output*ref_to_alt_embeddings
        
        

        catpooler_output=self.catpooler(alted_output,res_blok)
        # sequence_res_output=torch.cat([res_blok,pooled_output],dim=1) catpooler
        outputs = (sequence_output,catpooler_output,) + encoder_outputs[
            1:
...
class ResBlk(nn.Module):
    ...
    def forward(self, x):
        ...
        :return:
        """
        out = self.acf(self.bn1(self.conv1(x)))
...
class ResNet_sub(nn.Module):
    def __init__(self):
        super().__init__()
        self.blk1 = ResBlk(12, 128, stride=4) # [b, 64, h, w] => [b, 128, h ,w]
        self.blk2 = ResBlk(128, 256, stride=2)
        self.avgPool=nn.AdaptiveAvgPool2d([1, 1])
        self.outlayer = nn.Linear(256*1*1, 768) 
    def forward(self, x):
        """
        :param x:
        :return:
        """
        # [b, 64, h, w] => [b, 1024, h, w] ,[16, 12, 12, 64]
        x = self.blk1(x)
        x = self.blk2(x)    
...
class ResNet_seq(nn.Module):
    def __init__(self):
        super().__init__()
        self.blk1 = ResBlk(4, 128, stride=1) # [b, 64, h, w] => [b, 128, h ,w]
        self.blk2 = ResBlk(128, 256, stride=2)
        self.avgPool=nn.AdaptiveAvgPool2d([1, 1])
        self.outlayer = nn.Linear(256*1*1, 768) 
    def forward(self, x):
...
class BertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.bert = BertModel(config)
        # self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(int(config.hidden_size*2), 2)
    ...
    Examples::
        ...
        if get_which =='att':
            ...
            if tmp_score is None:
                ...
                if (att_head_mask is not None) and (labels is not None):
                ...
                gradient = torch.autograd.grad(torch.unbind(prob[:, labels]), tmp_score)
                return gradient[0],prob[:, labels]
        else:
            outputs = self.bert(
                input_ids,
                input_ids2,
